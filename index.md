---
layout: default
---
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@48,400,0,0" />


## 👋🏻 Greetings 
<hr class='section'>
<img class="profile-picture" src="asset/cat-profile_34.png">

Hi, My  name is Wonhee GO.  
As an NLP researcher, I am a good questioner.  

With my deep interest in the field of NLP, I am delighted to continuously strive to devise effective strategies and develop techniques that enhance the quality of my work output. 

Follow [stellarway](https://github.com/stellarway), and you will find a stellar way to reach your destination!🌟
<br>
<br>

---

#### About me

Allow me to introduce myself by highlighting three of my key strengths:  
**Ideation**, **strategic thinking**, and **maximizing results**.

* *Ideation* - With my quick-witted creativity, I able to come up with innovative solutions for my projects. It helps me reach to unique perspective and explore new possibilities on my work.

* *Strategic thinking* - I analyze complex problems and develop plans to achieve my goals. I believe that my ability to balance creativity with practicality is one of my greatest strengths.

* *Maximizing results* - As a maximizer, I am always seeking top-notch not only in my work but also personal development. I continually explore the best way to reach my goals improving my perfomance and expanding my capabilities. This dedication to excellence enables me to acheive victorious outcomes.

I would like to share reliable CliftonStrengths report(<a href="asset/signature-theme-65053644-en.pdf">EN</a>/<a href="asset/signature-theme-65053644-ko.pdf">KO</a>), published by Gallup. This fruitful report provides valuable insight into my traits, which contribute to the successful consequence in my field.

<br>
<br>

## 😆 Research Interest
<hr class='section'>

#### On my work

<a class="tag" href="#">NLP</a>
<a class="tag" href="#">MultiModal</a>
<a class="tag" href="#">NMT</a>
<a class="tag" href="#">OCR</a>
<a class="tag" href="#">구어체</a>
<a class="tag" href="#">QE</a>
<a class="tag" href="#">DA_score</a>
<a class="tag" href="#">probing_task</a>
<a class="tag" href="#">ML</a>
<a class="tag" href="#">Terminology</a>
<a class="tag" href="#">data</a>
<a class="tag" href="#">NER</a>
<a class="tag" href="#">비식별화</a>
<a class="tag" href="#">DP</a>
<a class="tag" href="#">TD</a>
<a class="tag" href="#">crawler</a>
<a class="tag" href="#">corpus</a>

---

#### Personal focus
<a class="tag" href="#">NLP</a>
<a class="tag" href="#">AIethics</a>
<a class="tag" href="#">MultiModal</a>
<a class="tag" href="#">verbalNLP</a>
<a class="tag" href="#">TechnicalWriting</a>



<!-- ## Publications

1. F.Bar, J.Doe: Effects of having a placeholder of a name
2. S.Holmes, J.Watson: Consequences of living with a sociopath in London -->
<br>


## 🔥 Projects
<hr class='section'>

For further details, please visit the '<a href="/portfolio">**Portfolio**</a>' tab.  
For your convenience, you can also access the **Korean version** of this section through the '<a href="/portfolio">**Portfolio**</a>' tab.  

<br>

Date|Project
----|-------
2023.01.11 - 2023.05.31 | Multimodal-Based K-Webtoon Translation System and Parallel Data<br><span class="subtitle">멀티모달 기반 웹툰 전용 번역기 연구 및 웹툰 병렬 데이터</span>
2022.08.08 - 2022.12.21 | TwiQE: Building and Applying QE Models for KO-EN/EN-KO Parallell Corpus<br><span class="subtitle">TwiQE: 한영/영한 번역 병렬 말뭉치 품질 예측 모델 구축 및 적용</span>
2022.04.27 - 2022.05.18 | Development of a Terminology Extraction System<br><span class="subtitle">용어 추출기 개발</span>
2022.02.10 - 2022.05.23 |AI Data Construction Project ‘Korean-English Colloquial and Technical Science Translation Corpus’ and  ‘Broadcasting and Specialized Multilingual Translation Corpus’<br><span class="subtitle">인공지능(AI) 학습용 데이터 구축 사업 ‘한영 구어체 및 기술과학 번역 말뭉치’구축 및 ‘방송 및 전문분야 다국어 번역 말뭉치’ 구축</span>
2021.12.29 - 2022.01.17 | Development of a De-Identification Model for Text Sentence<br><span class="subtitle">텍스트 문장 비식별화 모델 개발</span>
2021.07.13 - 2022.02.07 | Development of an NER Model and NER Data Construction<br><span class="subtitle">NER 모델 개발 및 데이터 구축</span>
2021.06.15 - 2021.09.27 | Provision of Information for the TTA Global Standardization Organization's System<br><span class="subtitle">TTA 글로벌 사실표준화기구 정보 제공 시스템 구축 용역</span>
2020.11.05 - 2021.04.23 | Research on tagMT<br><span class="subtitle">tagMT 연구</span>
2020.09.22 - 2020.12.31 | AI Data Construction Project ‘English Translation Corpus’<br><span class="subtitle">인공지능(AI) 학습용 데이터 구축 사업 ‘영어번역말뭉치 AI 데이터’ 구축</span>
2020.09.17 - 2021.04.24 | Research on NER and NER Data Construction<br><span class="subtitle">NER 리서치 및 데이터 구축</span>
2020.06.30 - 2020.08.26 | Term Dictionary(TD) Inspection and Construction<br><span class="subtitle">TD 관리 점검 및 구축</span>
2020.06.03 - 2020.06.26 | Development of Algorithms for Parallel Corpus Inspection and Refinement<br><span class="subtitle">병렬 말뭉치 정제 및 검수 알고리즘 작성</span>
2020.05.26 - 2020.06.05 | Development of Parallel Corpus Crwaler<br><span class="subtitle">병렬 말뭉치 크롤러 개발</span>

<br>
<br>

## 💻 Work Experience
<hr class='section'>
<span class="date-info">2020.05.25 - 2023.05</span>&emsp;<span class="location">Seoul, South Korea</span><br>
**Twigfarm**<a class="material-symbols-outlined outlink" target="_blank" href="https://en.twigfarm.net">call_made</a>&emsp;NLP research engineer
<br>
<br>

## 🎓 Education
<hr class='section'>

#### University
<span class="date-info">2010.03.01 - 2016.02.29</span>&emsp;<span class="location">Seoul, South Korea</span><br>
**Ewha womans university**<a class="material-symbols-outlined outlink" target="_blank" href="http://www.ewha.ac.kr/ewhaen/index.do">call_made</a>&emsp;Chemistry & Nanoscience(BS)&emsp;3.34/4.3 (3.64/4.5)
<br>
<br>

---

#### Training
<span class="date-info">2019.05.23 - 2019.11.13</span>&emsp;<span class="location">Seoul, South Korea</span><br>
**인공지능 자연어처리(NLP) 기반 기업 데이터 분석 개발자 양성 과정**<a class="material-symbols-outlined outlink" target="_blank" href="https://onoffmix.com/event/177892">call_made</a>  
- 사업명 : 혁신성장 청년인재 집중양성 
- 주최주관 : 과학기술정보통신부/ 정보통신기획평가원(IITP)  
- 교육기관 : 삼성 계열 기업교육 전문 기업 멀티캠퍼스

<br>
<br>

## 🏅 Awards and Honors
<hr class='section'>
<span class="date-info">2022.12.21</span>  
**2022 AI 데이터 품질 개선 오픈랩**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.datanet.co.kr/news/articleView.html?idxno=179709">call_made</a>   
- 주최주관 : 과학기술정보통신부/ 한국지능정보사회진흥원(NIA)  
- 입상내역 : 우수논문(상위 3개팀 내)  
- 상세내용 : 번역문 품질 평가(QE) DA 점수 예측 모델을 제안하고, 한-영/영-한 벙렬 말뭉치 총 4,185,507문장 쌍에 대해 분석을 실시하여 전반적으로 품질이 좋지 않은 데이터셋을 찾아냄. 정보과학회의 초청을 받아 논문을 제출한 상태이며, 관련 특허를 출원한 상태.

<br>
<br>

## 🔤 Language Skills
<hr class='section'>

#### Natural Languages

Language|Reading|Writing|Speaking|Test(score)
--------|-------|-------|--------|---
**Korean**|Native|Native|Native|KBS한국어능력(2-)<br><span class="subtitle">2020.05.27</span>
**English**|Advanced|Intermediate|Basic|TOEFL(90)<br><span class="subtitle">2019.05.11</span>
**Japanese**|Novice|Novice|Novice|JLPT(N5)<br><span class="subtitle">2022.01.13</span>

<br>
Guideline for Levels of language proficiency is below:
<p style="font-size:small;">
1. <b>Novice</b>: Can recognize some high-frequency words or phrases related to basic needs, but is not yet able to produce them independently.<br>
2. <b>Basic</b>: Can understand and use simple phrases and sentences, express basic needs and opinions, and understand simple written materials.<br>
3. <b>Intermediate</b>: Can communicate on familiar and routine tasks, describe experiences, events, and ambitions, and understand the main points of written materials on familiar topics.<br>
4. <b>Advanced</b>: Can handle complex language tasks and understand professional topics, express ideas and opinions fluently, and understand and summarize complex written materials.<br>
5. <b>Fluent/Native</b>: Can communicate effectively and accurately in all situations, understand and use idiomatic expressions and cultural references, and read and write complex materials with ease.
</p>

---

<br>

#### Langauage

Language|Reading|Writing|Conversation
--------|------------|-------|-------
**Python**|Fluent|Fluent|Fluent

<br>
<br>

## 🪪 Certifications
<hr class='section'>
<span class="date-info">2022.07.15</span>  
**빅데이터분석기사**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.dataq.or.kr/www/sub/a_07.do">call_made</a> 
- 발급기관 : 한국데이터산업진흥원
- 자격내용 : 국가기술자격
- 증명내용 : 빅데이터 이해를 기반으로 빅데이터 분석 기획, 빅데이터 수집, 저장, 처리 및 빅데이터 분석과 시각화 수행 능력을 증명

---

<span class="date-info">2021.06.18</span>  
**데이터분석 준전문가(ADsP, Advanced Data Analytics Semi-Professional)**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.dataq.or.kr/www/sub/a_06.do">call_made</a> 
- 발급기관 : 한국데이터산업진흥원
- 자격내용 : 국가공인자격
- 증명내용 : 데이터 이해에 대한 기본지식을 바탕으로, 데이터분석 기획 및 데이터분석 등의 직무의 수행 능력을 증명

---

<span class="date-info">2019.12.31</span>  
**SQL 개발자**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.dataq.or.kr/www/sub/a_06.do">call_made</a> 
- 발급기관 : 한국데이터산업진흥원
- 자격내용 : 국가공인자격
- 증명내용 : 데이터베이스와 데이터 모델링에 대한 지식을 바탕으로 소프트웨어 개발 시 데이터를 조작하고 추출하는 데 있어 정확하고 최적의 성능을 발휘하는 SQL을 작성할 수 있음을 증명

<br>
<br>

## 🤓 Teaching Experience
<hr class='section'>

#### Guest lecture
<span class="date-info">2022.07.13 - 2022.07.18</span><br>
**한국외국어대학교 데이터청년캠퍼스 | 자연어처리 기반 딥러닝기술 융합과정 강의**<a class="material-symbols-outlined outlink" target="_blank" href="https://dataonair.or.kr/bigjob/company_info/">call_made</a><span class="subtitle">(총 32시간/4회차)</span>
<br><br>
*기본적인 언어 모델 구조와 Transformer 강의 및 Pytorch 기반 코드 실습 진행*

사업명 | 데이터 청년 캠퍼스 
과정명 | 자연어처리 기반 딥러닝기술 융합과정
주최주관 | 과학기술정보통신부/ 한국데이터산업진흥원
교육기관 | 한국외국어대학교  
과정기간 | 2022.06.27 - 2022.08.31
대상자 | 데이터 관련 전공 대학생(3~4학년) 혹은 비전공 석박사 대학원생 

---

<span class="date-info">2021.07.06 - 2021.07.20</span><br>
**한국외국어대학교 데이터청년캠퍼스 | 자연어처리 기반 딥러닝기술 융합과정 중 현업 전문가 실무 특강**<a class="material-symbols-outlined outlink" target="_blank" href="https://dataonair.or.kr/bigjob/company_info/">call_made</a><span class="subtitle">(총 5시간/2회차)</span>
<br><br>
*자연어처리 실무 이해, Transformer 개론 및 기계번역 모델링 실습*

사업명 | 데이터 청년 캠퍼스 
과정명 | 자연어처리 기반 딥러닝기술 융합과정
주최주관 | 과학기술정보통신부/ 한국데이터산업진흥원
교육기관 | 한국외국어대학교  
과정기간 | 2021.06.28 - 2021.08.31
대상자 | 데이터 관련 전공 대학생(3~4학년) 혹은 비전공 석박사 대학원생<br><span class="subtitle">외대생 58명/ 타대생 18명, 총 75명</span>
  
---

<span class="date-info">2021.05.29 - 2021.07.10</span><br>
**혁신성장 청년인재 집중양성 | 인공지능 자연어처리[NLP]기반 기업 데이터 분석 과정 특강**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.multicampus.com/em/enrolment/courseDetai?p_menu=NzUjU1VC&p_gubun=Qw==&corsCd=FA006N">call_made</a><span class="subtitle">(총 3시간/2회차)</span>
<br><br>
*포트폴리오 작성법과 Transformer 개론*

사업명 | 혁신성장 청년인재 집중양성 
과정명 | 인공지능 자연어처리[NLP]기반 기업 데이터 분석
주최주관 | 과학기술정보통신부/ 정보통신기획평가원(IITP)  
교육기관 | 삼성 계열 기업교육 전문 기업 멀티캠퍼스
과정기간 | 2021.05.10 - 2021.11.03
대상자 | 4년제 대학교 졸업자 혹은 졸업 예정자

---
<br>

#### Mentoring


<span class="date-info">2022.10.11 - 2022.12.30</span><br>
**트위그팜 현업 연구프로젝트 현장실습 | SOL프로젝트 3기**
<br><br>
*프로젝트 멘토링 및 기술 멘토링*  
산출물 : [구어체를 위한 NER 모델](https://github.com/twigfarm/letr-sol-NERSOTA)  

사업명 | 트위그팜 현업 연구프로젝트 현장실습
과정명 | SOL프로젝트
주최주관 | (주)트위그팜
과정기간 | 2021.08.04 - 2021.10.31
대상자 | 2021.10.04 - 2021.12.31

---

<span class="date-info">2022.08.09 - 2022.08.25</span><br>
**한국외국어대학교 데이터청년캠퍼스 | 자연어처리 기반 딥러닝기술 융합과정 멘토링**<a class="material-symbols-outlined outlink" target="_blank" href="https://dataonair.or.kr/bigjob/company_info/">call_made</a><span class="subtitle">(총 10회차)</span>
<br><br>
*프로젝트 멘토링 및 기술 멘토링*

사업명 | 데이터 청년 캠퍼스 
과정명 | 자연어처리 기반 딥러닝기술 융합과정
주최주관 | 과학기술정보통신부/ 한국데이터산업진흥원
교육기관 | 한국외국어대학교  
과정기간 | 2021.06.27 - 2021.08.31
대상자 | 데이터 관련 전공 대학생(3~4학년) 혹은 비전공 석박사 대학원생 
  
---

<span class="date-info">2022.04.30 - 2022.06.04</span><br>
**K-Digital Training | 서비스 산업 데이터를 활용한 빅데이터 분석 과정 멘토링**<a class="material-symbols-outlined outlink" target="_blank" href="/asset/KDT2021_multicampus.pdf">call_made</a><span class="subtitle">(총 3회차)</span>
<br><br>
*프로젝트 멘토링, 기술 멘토링 및 커리어 설계 가이드*

사업명 | K-Digital Training(첨단산업 디지털 핵심 실무인재 양성훈련)  
과정명 | 서비스 산업 데이터를 활용한 빅데이터 분석
주최주관 | 고용노동부  
교육기관 | 삼성 계열 기업교육 전문 기업 멀티캠퍼스
과정기간 | 2021.01.03 - 2021.06.10

---

<span class="date-info">2021.10.04 - 2021.12.31</span><br>
**트위그팜 현업 연구프로젝트 현장실습 | SOL프로젝트 2기**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.letr.ai/blog/story-20220121">call_made</a>
<br><br>
*프로젝트 멘토링, 기술 멘토링 및 모델 검증*  
산출물 : [번역기 품질 평가](https://github.com/twigfarm/letr-sol-mt-eval)  

사업명 | 트위그팜 현업 연구프로젝트 현장실습
과정명 | SOL프로젝트
주최주관 | (주)트위그팜
과정기간 | 2021.08.04 - 2021.10.31
대상자 | 2021.10.04 - 2021.12.31


---

<span class="date-info">2021.08.09 - 2021.08.27</span><br>
**한국외국어대학교 데이터청년캠퍼스 | 자연어처리 기반 딥러닝기술 융합과정 멘토링**<a class="material-symbols-outlined outlink" target="_blank" href="https://dataonair.or.kr/bigjob/company_info/">call_made</a><span class="subtitle">(총 4회차)</span>
<br><br>
*프로젝트 멘토링 및 기술 멘토링*

사업명 | 데이터 청년 캠퍼스 
과정명 | 자연어처리 기반 딥러닝기술 융합과정
주최주관 | 과학기술정보통신부/ 한국데이터산업진흥원
교육기관 | 한국외국어대학교  
과정기간 | 2021.06.28 - 2021.08.31
대상자 | 데이터 관련 전공 대학생(3~4학년) 혹은 비전공 석박사 대학원생<br><span class="subtitle">외대생 58명/ 타대생 18명, 총 75명</span>
  
---

<span class="date-info">2021.08.04 - 2021.10.31</span><br>
**트위그팜 현업 연구프로젝트 현장실습 | SOL프로젝트 1기**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.letr.ai/blog/story-20211126-1">call_made</a>
<br><br>
*프로젝트 멘토링 및 기술 멘토링*  
산출물 : [욕설혐오표현 분류](https://github.com/twigfarm/letr-sol-profanity-filter)  

사업명 | 트위그팜 현업 연구프로젝트 현장실습
과정명 | SOL프로젝트
주최주관 | (주)트위그팜
과정기간 | 2021.08.04 - 2021.10.31
대상자 | 자연어처리 관련 교육 프로그램을 수료했거나 수료 예정자

---

<span class="date-info">2021.05.29 - 2021.10.30</span><br>
**혁신성장 청년인재 집중양성 | 인공지능 자연어처리[NLP]기반 기업 데이터 분석 과정 멘토링**<a class="material-symbols-outlined outlink" target="_blank" href="https://www.multicampus.com/em/enrolment/courseDetai?p_menu=NzUjU1VC&p_gubun=Qw==&corsCd=FA006N">call_made</a><span class="subtitle">(총 12회차)</span>
<br><br>
*프로젝트 멘토링, 기술 멘토링, 현업 인사이트 공유 및 진로 상담*

사업명 | 혁신성장 청년인재 집중양성 
과정명 | 인공지능 자연어처리[NLP]기반 기업 데이터 분석
주최주관 | 과학기술정보통신부/ 정보통신기획평가원(IITP)  
교육기관 | 삼성 계열 기업교육 전문 기업 멀티캠퍼스
과정기간 | 2021.05.10 - 2021.11.03
대상자 | 4년제 대학교 졸업자 혹은 졸업 예정자

<br>
<br>




