...
하지만 이 **리스트 형태로 반환**되는 성질 때문에 더 쉽게 풀리는 코드도 있다. 위에서 제목 가져온 코드는 select함수로 다음과 같다.

```py
for title in soup.select('ul dt a'):
	title = title.get('title')
```

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzc0NDU3NTRdfQ==
-->